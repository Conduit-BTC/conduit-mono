...
const COMPLETION_MARKER_PREFIX = "conduit:merchant:event-handoff-complete:v1"
const COMPLETION_MARKER_PENDING = "pending"
const COMPLETION_MARKER_CONFIRMED = "complete"
...
/**
 * Keep authenticated ACK authority separate from discovery diagnostics.
 * A degraded read can make absence uncertain, but it cannot erase an exact
 * ACK already found and validated for this receipt and graph.
 */
...
>(input: {
  ...
  const blocker: MerchantHandoffAckReadBlocker | null = !input.read
    ? input.isError
      ? "read_error"
      : input.isFetching
        ? "pending"
  ...
  const evidence = input.read
    ...
    : { exactAck: null, conflicting: false }
  return {
    ...evidence,
    blocker,
...
function completionMarkerKey(
  principalPubkey: string,
  readyReceiptId: string,
  orderCorrelationRef: string
): string {
  return `${COMPLETION_MARKER_PREFIX}:${principalPubkey.trim().toLowerCase()}:${readyReceiptId}:${orderCorrelationRef}`
}

function completionMarkerKeyPrefix(principalPubkey: string): string {
  return `${COMPLETION_MARKER_PREFIX}:${principalPubkey.trim().toLowerCase()}:`
}

function hasCompletedReadyReceiptMarker(
  principalPubkey: string,
  ...
  return (
    ...
    typeof delivery.record.orderCorrelationRef === "string" &&
    [COMPLETION_MARKER_PENDING, COMPLETION_MARKER_CONFIRMED].includes(
      storage.getItem(
        completionMarkerKey(
          principalPubkey,
...
export function loadEventMarketHandoffDeliveries(
  ...
  try {
    ...
    return Array.from(latest.values()).filter(
      (delivery) =>
        !hasCompletedReadyReceiptMarker(principalPubkey, delivery, storage)
    )
  } catch {
    return []
  }
...
function readStoredDeliveriesRaw(
  principalPubkey: string,
  storage: Pick<Storage, "getItem">
): StoredEventMarketHandoffDelivery[] {
...
function readStoredDeliveries(
  ...
  return readStoredDeliveriesRaw(principalPubkey, storage).filter(
    (delivery) =>
      !hasCompletedReadyReceiptMarker(principalPubkey, delivery, storage)
  )
}

function cleanupCompactedCompletionMarkers(
  principalPubkey: string,
  storage: HandoffStorage
): void {
  try {
    const retained = readStoredDeliveriesRaw(principalPubkey, storage)
    const prefix = completionMarkerKeyPrefix(principalPubkey)
    ...
    for (const key of keys) {
      ...
      const identity = key.slice(prefix.length).split(":")
      if (identity.length !== 2) continue
      const [readyReceiptId, orderCorrelationRef] = identity
      const stillRetained = retained.some(
        (delivery) =>
          delivery.record.messageType === "organizer_fulfillment_receipt" &&
          delivery.record.readyReceiptId === readyReceiptId &&
          delivery.record.orderCorrelationRef === orderCorrelationRef
      )
      if (!stillRetained) storage.removeItem(key)
    }
  } catch {
    // Marker compaction is best-effort and must not turn an already durable
    // exact-wrap checkpoint into a false persistence failure.
  }
}

function completedReadyReceiptIsCompacted(
  principalPubkey: string,
  readyReceiptId: string,
  orderCorrelationRef: string,
  storage: Pick<Storage, "getItem">
  ...
  try {
    return !readStoredDeliveriesRaw(principalPubkey, storage).some(
      (delivery) =>
        delivery.record.messageType === "organizer_fulfillment_receipt" &&
        delivery.record.readyReceiptId === readyReceiptId &&
        delivery.record.orderCorrelationRef === orderCorrelationRef
...
function upsertDelivery(
  ...
  try {
    ...
    cleanupCompactedCompletionMarkers(principalPubkey, storage)
  } catch {
    throw new Error(
...
/**
 * Mark the exact ready-receipt retry seed releasable after its order has
 * reached a successfully published terminal `complete` status. The marker is
 * independent of the shared delivery array, so this terminal transition cannot
 * overwrite a concurrent retry/revocation update from another tab. A two-step
 * marker handshake lets a concurrent upsert compact the marked row; this
 * writer treats a collected marker as success only after the raw row is absent.
 * The upsert removes either handshake state after compaction. Retryable ready
 * rows and all revocation rows stay durable.
 */
export function releaseCompletedEventMarketHandoffReceipt(
  principalPubkey: string,
  readyReceiptId: string,
  orderCorrelationRef: string,
  storage: HandoffStorage | null = browserStorage()
  ...
  try {
    ...
    const matches = (delivery: StoredEventMarketHandoffDelivery) =>
      delivery.record.messageType === "organizer_fulfillment_receipt" &&
      delivery.record.readyReceiptId === readyReceiptId &&
      delivery.record.orderCorrelationRef === orderCorrelationRef
    const ready = deliveries.find(matches)
    if (!ready || eventMarketHandoffDeliveryNeedsRetry(ready)) return false
    const markerKey = completionMarkerKey(
      principalPubkey,
      readyReceiptId,
      orderCorrelationRef
    )
    storage.setItem(markerKey, COMPLETION_MARKER_PENDING)
    if (storage.getItem(markerKey) !== COMPLETION_MARKER_PENDING) {
      return completedReadyReceiptIsCompacted(
        principalPubkey,
        readyReceiptId,
        orderCorrelationRef,
        storage
    ...
    storage.setItem(markerKey, COMPLETION_MARKER_CONFIRMED)
    if (storage.getItem(markerKey) === COMPLETION_MARKER_CONFIRMED) {
      if (
        completedReadyReceiptIsCompacted(
          principalPubkey,
          readyReceiptId,
          orderCorrelationRef,
          storage
...
export async function issueOrganizerReadyReceipt(input: {
  ...
  if (!input.authorizationConfirmed) {
    throw new Error(
      "Explicitly confirm payment is settled, the order is ready, and organizer release is authorized before sharing this receipt."
    )
  }
