...
export function EventProductPublisherDialog({
  ...
  onPublished: (productCoordinate: string) => void | Promise<void>
}) {
  ...
  const retryMutation = useMutation({
    mutationFn: () => {
      ...
      return retryEventProductDelivery(signedEvent, merchantPubkey)
    },
    onMutate: () => {
      setActionError("")
      setActionState("publishing")
    },
    onSuccess: () => setActionState("success"),
    onError: (error) => {
      setActionState("error")
      setActionError(
      ...
      >
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            setSubmitted(true)
            if (!validation.canPublish || pending) return
        ...
        >
          <div className="grid gap-1.5">
            ...
            {templatesQuery.isError && (
              <p className="text-xs leading-5 text-[var(--text-muted)]">
                Existing products could not be loaded. You can still start with
                a blank product.
              </p>
            )}
          </div>
          ...
          <div className="grid gap-1.5">
            ...
            />
            <FieldError id="event-product-title-error" message={errors.title} />
          </div>
          ...
          <fieldset className="grid gap-2">
            ...
            >
              <span>
                ...
                <span className="mt-1 block text-xs leading-5 text-[var(--text-muted)]">
                  You confirm payment and mark the item ready. Stay available
                  remotely; the organizer receives your signed handoff
                  instruction and pickup code, not independent payment proof.
                </span>
              </span>
            </Button>
          ...
          <DialogFooter>
            ...
            >
              {actionState === "success" ? "Done" : "Cancel"}
            </Button>
            {actionState !== "success" && (
