  ...
  return (
    <Card data-testid="organizer-handoff-receipt-queue">
      ...
      <CardContent className="space-y-3">
        ...
        {evidenceBlocked && (!loading || claims.length > 0) && (
          <p
            className="rounded-xl border border-warning/30 bg-warning/10 px-3 py-2 text-sm leading-6 text-[var(--text-primary)]"
            role="alert"
          >
            Current receipt evidence could not be verified completely. Handoff
            actions are blocked until a fresh, conflict-free read succeeds.
          </p>
        )}
