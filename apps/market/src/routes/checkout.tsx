...
function CheckoutPage() {
  ...
  const wallet = useWallet({
    enabled: paymentPathEnabled,
    refreshBalance: paymentPathEnabled,
  })
  const total = useMemo(() => {
  ...
  useEffect(() => {
    ...
    if (!paymentPathEnabled || !merchantLud16) {
  ...
  }, [merchantLud16, paymentPathEnabled])
  ...
  const canAttemptLightningPayment =
    paymentPathEnabled && (canTrySavedNwcWallet || weblnAvailable)
  ...
  const allowsManualLightningFallback =
    paymentPathEnabled && !!merchantLud16 && lnurlReadyForSelectedPayment
  ...
  async function assertCheckoutItemsAvailable(): Promise<CartItem[]> {
  ...
  async function placeOrder(): Promise<void> {
    ...
    if (!signedBuyerPubkey && !(isGuestCheckout && verifiedZeroCostPickup)) {
      return
    }
    if (checkoutEvidenceIsChecking) {
      setError(
    ...
    try {
      ...
      const [delivery] = await Promise.all([
        submitBuyerOrderMessage({
          ...
          ndk,
          merchantPubkey: selectedMerchant,
          buyer: guestIdentity ?? buyerPubkey,
          lifecycle: {
            orderId,
            createdAt: orderCreatedAt,
            buyerPubkey,
  ...
  async function payNow(): Promise<void> {
    ...
    try {
      ...
      if (orderDelivered) void runOrderPayment(serviceCtx)
              ...
              <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 sm:p-6">
                ...
                {!showFastCheckoutSurface && !lnurlProbing && (
                  <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-elevated)] p-5">
                    ...
                    {paymentPathEnabled && wallet.status === "disconnected" && (
