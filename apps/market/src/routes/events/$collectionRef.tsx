    ...
    pickups.length === 0
      ...
      : pickups.length === 1
        ? [pickups[0]!.title, pickups[0]!.location ?? pickups[0]!.geohash]
            .filter(Boolean)
            .join(" / ")
        : `${pickups.length} signed pickup options; each product shows its exact handler.`
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant="outline" className="gap-1.5">
                <Radio className="h-3.5 w-3.5" /> Organizer-signed
              </Badge>
            </div>
          ...
          <aside className="space-y-5">
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-elevated)] p-5">
              ...
              <p className="mt-3 text-xs leading-5 text-[var(--text-muted)]">
                This identity signed the event and collection. Each pickup
                option is attributed to its exact signer. Conduit does not
            ...
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-elevated)] px-5 py-2">
              <EvidenceRow label="Calendar" eventId={calendar.eventId} />
              <EvidenceRow label="Collection" eventId={collection.eventId} />
