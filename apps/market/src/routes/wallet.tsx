...
function WalletPage() {
  ...
  async function handleConnect(): Promise<void> {
    ...
    if (!trimmed.startsWith("nostr+walletconnect://")) {
      setInputError("The connection string must start with nostr+walletconnect://")
      return
    }
    setInputError(null)
    setPending(true)
        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            <p className="mt-4 max-w-xl text-base leading-7 text-[var(--text-secondary)]">
              ...
              enable fast checkout. Your wallet connection is private and stored
              only on this device.
            </p>
          </div>
          ...
          <div>
            ...
            <div className="mt-3 rounded-[1.75rem] border border-[var(--border)] bg-[color-mix(in_srgb,var(--primary-500)_1%,transparent)] px-5 py-4 shadow-[var(--shadow-glass-inset)]">
                ...
                >
                  <dl className="m-0">
                    <div className="flex items-center justify-between gap-4 px-4 py-3">
                      <dt className="shrink-0 text-xs text-[var(--text-muted)]">Wallet pubkey</dt>
                      <dd
                        className="group/pubkey relative min-w-0 cursor-default font-mono text-xs text-[var(--text-secondary)]"
                      >
                    ...
                    <div className="flex items-center justify-between gap-4 px-4 py-3">
                      <dt className="shrink-0 text-xs text-[var(--text-muted)]">Relay</dt>
                    ...
                    {wallet.info?.alias && (
                      <div className="flex items-center justify-between gap-4 px-4 py-3">
                        <dt className="shrink-0 text-xs text-[var(--text-muted)]">Alias</dt>
                        <dd className="text-xs text-[var(--text-secondary)]">{wallet.info.alias}</dd>
                      </div>
                    )}
                    {wallet.info?.network && (
                      <div className="flex items-center justify-between gap-4 px-4 py-3">
                        <dt className="shrink-0 text-xs text-[var(--text-muted)]">Network</dt>
                        <dd className="text-xs capitalize text-[var(--text-secondary)]">{wallet.info.network}</dd>
                      </div>
                    )}
                    {wallet.info?.methods && wallet.info.methods.length > 0 && (
                      <div className="flex items-start justify-between gap-4 px-4 py-3">
                        <dt className="shrink-0 text-xs text-[var(--text-muted)]">Capabilities</dt>
              ...
              {wallet.status === "error" && wallet.error && (
                <StatusPill variant="error" className="mt-4 w-full justify-start rounded-2xl px-4 py-3 text-sm">
                  {wallet.error}
                </StatusPill>
              )}
              ...
              {wallet.status === "unsupported" && (
                <div className="mt-4 rounded-2xl border border-[var(--warning)] bg-[color-mix(in_srgb,var(--warning)_10%,transparent)] p-3 text-sm text-[var(--warning)]">
                  ...
                  <code className="font-mono">pay_invoice</code>. Fast checkout
                  will remain unavailable until you connect a payment-capable
                  wallet.
                </div>
              )}
          ...
          {!isConnected && (
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--text-secondary)]">
                Paste a <code className="font-mono text-xs">nostr+walletconnect://</code>{" "}
          ...
          {isConnected && (
            <div className="rounded-[1.5rem] border border-dashed border-[var(--border)] bg-[var(--surface)] p-4 text-sm leading-6 text-[var(--text-secondary)]">
              To use a different wallet, disconnect the current one above, then
              paste a new connection string.
            </div>
          )}
          ...
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--text-secondary)]">
              Fast checkout lets you pay merchants directly at checkout without
              waiting for a manual invoice.
            </p>
            ...
            <div className="mt-3 rounded-[1.75rem] border border-[var(--border)] bg-[color-mix(in_srgb,var(--primary-500)_1%,transparent)] px-5 py-4 shadow-[var(--shadow-glass-inset)]">
              <ol className="space-y-3 text-sm leading-7 text-[var(--text-secondary)]">
                ...
                <li>
                  2. At checkout, Conduit checks if the merchant has a Lightning
                  address and your wallet supports outgoing payments.
                </li>
                <li>
                ...
                <li>
                  4. If fast checkout is not available, the standard order-first
