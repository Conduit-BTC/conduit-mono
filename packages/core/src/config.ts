 ...
 * registry for temporary compatibility writes of validated kind-16 order
...
export interface ConduitConfig {
  ...
  /** Redeploy-controlled speculative kind-10050 warming for cart merchants. */
...
function getViteEnv(): {
  ...
  checkoutOrderRoutePrefetch: string
  nip89RelayHint: string
...
const commerceDmFallbackRelaySet = new Set(commerceDmFallbackRelayUrls)
...
const checkoutOrderRoutePrefetchEnabled = ["1", "true", "on"].includes(
  env.checkoutOrderRoutePrefetch.trim().toLowerCase()
)
...
export function getRelayBucketConfigs(
  ...
  return [
    ...
    {
      id: "dm_compatibility_order",
      label: "Validated-order compatibility route",
      relayUrls: cfg.dmCompatibilityOrderRelayUrls,
    },
    {
      id: "zap_public",
