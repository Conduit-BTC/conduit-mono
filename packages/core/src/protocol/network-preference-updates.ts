...
function normalizedContentMatches(input: {
  ...
  desiredTags: string[][]
  usableState: string
  coordinatedPending?: boolean
}): boolean {
  const contentMatches =
    JSON.stringify(input.currentTags) === JSON.stringify(input.desiredTags)
  const coordinatedPending =
    input.state === "distribution_pending" && input.coordinatedPending === true
  const stateMatches = input.state === input.usableState || coordinatedPending
  if (input.desiredTags.length === 0) {
    return (
      (input.state === "signed_empty" || coordinatedPending) && contentMatches
    )
  }
  return stateMatches && contentMatches
}
...
async function publishUnderLock(input: {
  ...
  const needsInbox =
    ...
    !normalizedContentMatches({
      ...
      usableState: "declared",
      coordinatedPending:
        reconciliation.inboxDeclaration.pendingWriteAuthorized === true,
    })
  ...
  for (const draft of drafts) {
    ...
    const relayPlan = await resolveRelayPlan(
      input.pubkey,
      draft.kind,
      signedEvent,
      excludedRelayUrls,
    ...
    signedCheckpoints.push({
      ...
      relayPlan: [...relayPlan],
      relayOutcomes: relayPlan.map((relayUrl) => ({
        relayUrl,
