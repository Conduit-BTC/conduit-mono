...
function explicitRemovalRelayUrlsForOperation(
  ...
  // A caller may hold an older durable snapshot while another tab advances
  // the process projection. Restrictions are monotonic for one operation;
  // a later re-add can take effect on the next durable decision.
  return ownerRelayUrls([
    ...cutoff.excludedRelayUrls,
    ...getInboxExplicitRemovalRelayUrls(accountPubkey),
  ])
}
