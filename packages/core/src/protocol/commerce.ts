...
import { getEffectiveRoleRelayUrls } from "../config"
...
function getRoleRelayUrls(
  role: "merchant" | "commerce" | "general",
  purpose: "in" | "dm" = "in"
): string[] | undefined {
  const relayUrls = getEffectiveRoleRelayUrls(role, purpose)
  return relayUrls.length > 0 ? relayUrls : undefined
}

function getRoleRelayFallbackUrls(
  roles: Array<"merchant" | "commerce" | "general">,
  purpose: "in" | "dm" = "in"
): string[] | undefined {
  const relayUrls = roles.flatMap((role) =>
    getEffectiveRoleRelayUrls(role, purpose)
  )
  return relayUrls.length > 0 ? Array.from(new Set(relayUrls)) : undefined
}
...
async function fetchDeletionTimestamps(
  ...
  for (const filter of filters) {
    const fetched = await runFetchEventsFanout(filter, {
      relayUrls: getRoleRelayUrls("merchant"),
...
async function fetchPublicProductRecords(query: {
  ...
  const events = await runFetchEventsFanout(filter, {
    relayUrls: getRoleRelayFallbackUrls(["commerce", "general"]),
    connectTimeoutMs: 4_000,
...
export async function getMerchantStorefront(
  ...
  try {
    const rawEvents = await runFetchEventsFanout(
      ...
      {
        relayUrls: getRoleRelayFallbackUrls([
          "merchant",
          "commerce",
          "general",
        ]),
...
async function fetchParsedOrderMessages(
  ...
  try {
    ...
    const wrapped = await runFetchEventsFanout(filter, {
      relayUrls: getRoleRelayFallbackUrls(["merchant", "general"], "dm"),
      connectTimeoutMs: 4_000,
