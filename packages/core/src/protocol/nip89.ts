...
const appDefinitions: Record<ConduitAppId, ConduitNip89AppDefinition> = {
  market: createAppDefinition({
    ...
    name: "Conduit Market",
    about:
      "Buyer marketplace app for discovering products and sending orders over Nostr.",
...
export function getConduitNip89AppDefinition(
  appId: ConduitAppId
): ConduitNip89AppDefinition {
  return appDefinitions[appId]
}

export function getConduitNip89HandlerAddress(
  appId: ConduitAppId
): string | null {
...
export function appendConduitClientTag(
  tags: NostrTag[] | undefined,
  appId: ConduitAppId
): NostrTag[] {
  const nextTags = Array.from(tags ?? []).filter(
    (tag) => !isConduitClientTagForApp(tag, appId)
  )
