...
const PUBLISH_OUTCOME_STRENGTH = {
  pending: 0,
  timed_out: 1,
  rejected: 2,
  acked: 3,
} as const
const READBACK_OUTCOME_STRENGTH = {
  pending: 0,
  timed_out: 1,
  absent: 2,
  observed: 3,
...
export interface AccountNetworkPreferenceRuntimeSnapshot {
...
export interface AccountNetworkPreferenceUpdateRepository {
  ...
  /** Atomic runtime snapshot when the backing store can provide one. */
  getRuntimeSnapshot?(
    pubkey: string
  ): Promise<AccountNetworkPreferenceRuntimeSnapshot>
  getDurableFrontiers(
    pubkey: string
...
function updateRelayOutcome(
  ...
  if (input.publish) {
    ...
    const currentStrength = PUBLISH_OUTCOME_STRENGTH[outcome.publishStatus]
    const nextStrength = PUBLISH_OUTCOME_STRENGTH[input.publish]
  ...
  if (input.readback) {
    ...
    const currentStrength = READBACK_OUTCOME_STRENGTH[outcome.readbackStatus]
    const nextStrength = READBACK_OUTCOME_STRENGTH[input.readback]
    if (nextStrength >= currentStrength) {
      outcome.readbackStatus = input.readback
      outcome.readbackAttemptedAt = observedAt
      outcome.observedAt =
        input.readback === "observed" ? observedAt : undefined
    }
  }
}
...
function mergeExactReadbackEvidence(input: {
  ...
  const completeObservedAt =
    coverage === "complete"
      ? Math.max(
          ...input.checkpoint.relayOutcomes.map(
            (outcome) => outcome.readbackAttemptedAt!
          )
        )
      : undefined
  const lookupObservedAt = completeObservedAt ?? input.observedAt
...
function createDexieNetworkPreferenceUpdateRepository(): AccountNetworkPreferenceUpdateRepository {
  return {
    ...
    async getRuntimeSnapshot(pubkey) {
      const normalized = requireCanonicalPubkey(pubkey)
      return await db.transaction(
        ...
        async () => {
          const [storedRecord, ownerEvidence, inboxEvidence] =
            await Promise.all([
              db.networkPreferenceUpdates.get(normalized),
              db.ownerRelayListEvidence.get(normalized),
              db.inboxDeclarationEvidence.get(normalized),
            ])
          return {
            record: storedRecord
              ? validateAccountNetworkPreferenceUpdateRecord(storedRecord)
              : null,
...
function currentAccountPrivateMessageRelayCutoff(
  pubkey: string
): AccountPrivateMessageRelayCutoff {
  return {
    pubkey,
    excludedRelayUrls: getInboxExplicitRemovalRelayUrls(pubkey),
  }
}

function applyAccountNetworkPreferenceRuntimeSnapshot(
  pubkey: AccountNetworkPreferencePubkey,
  snapshot: AccountNetworkPreferenceRuntimeSnapshot
): {
  record: AccountNetworkPreferenceUpdateRecord | null
  relayCutoff: AccountPrivateMessageRelayCutoff
} {
  if (!snapshot.record) {
    setInboxCoordinatedPendingCheckpoint(pubkey, null)
    setInboxExplicitRemovalRelayUrls(pubkey, [])
    return {
      record: null,
      relayCutoff: currentAccountPrivateMessageRelayCutoff(pubkey),
    }
  }
  const record = validateAccountNetworkPreferenceUpdateRecord(snapshot.record)
  if (record.pubkey !== pubkey) {
    throw new Error("Network preference runtime snapshot changed accounts")
  }
  const frontiers = validateAccountNetworkPreferenceDurableFrontiers(
    pubkey,
    snapshot.frontiers
  )
  applyAccountNetworkPreferenceRuntimeState(
    record,
    snapshot.inboxEvidence,
    frontiers
  )
  return {
    record: clone(record),
...
async function readAccountNetworkPreferenceRuntimeSnapshot(
  pubkey: AccountNetworkPreferencePubkey,
  repository: AccountNetworkPreferenceUpdateRepository
): Promise<AccountNetworkPreferenceRuntimeSnapshot> {
  if (repository.getRuntimeSnapshot) {
    return await repository.getRuntimeSnapshot(pubkey)
  }
  const [record, frontiers] = await Promise.all([
    repository.get(pubkey),
    repository.getDurableFrontiers(pubkey),
  ])
  return { record, inboxEvidence: null, frontiers }
}

/**
 * Load the exact account's durable whole-removal policy immediately before a
 * private-message operation. Storage failure preserves a previously validated
 * restriction and fails closed instead of silently widening the relay set.
 */
export async function loadAccountPrivateMessageRelayCutoff(
  pubkey: string,
  ...
  // Non-browser tests and server-only consumers have no shared durable store.
  // Keep their existing process-only behavior without weakening browser I/O.
  if (
    repository === dexieAccountNetworkPreferenceUpdateRepository &&
    typeof indexedDB === "undefined"
  ) {
    const key = pubkey.trim().toLowerCase()
    return currentAccountPrivateMessageRelayCutoff(key)
  }
  const normalized = requireCanonicalPubkey(pubkey)
  try {
    const snapshot = await readAccountNetworkPreferenceRuntimeSnapshot(
      normalized,
      repository
    )
    return applyAccountNetworkPreferenceRuntimeSnapshot(normalized, snapshot)
      .relayCutoff
  } catch (error) {
    // Never retain a stale pending-write capability when storage cannot prove
    // it. Keep a previously validated removal because it is a restriction.
    setInboxCoordinatedPendingCheckpoint(normalized, null)
    throw error
  }
}

export interface AccountNetworkPreferenceRuntimeStateObserver {
  onChange(input: {
    record: AccountNetworkPreferenceUpdateRecord | null
    relayCutoff: AccountPrivateMessageRelayCutoff
  }): void
...
/** Keep process projections synchronized with committed changes from any tab. */
export function subscribeToAccountNetworkPreferenceRuntimeState(
  pubkey: string,
  observer: AccountNetworkPreferenceRuntimeStateObserver
): () => void {
  const normalized = requireCanonicalPubkey(pubkey)
  const readSnapshot =
    dexieAccountNetworkPreferenceUpdateRepository.getRuntimeSnapshot
  if (!readSnapshot) {
    observer.onError(
      new Error("Network preference runtime snapshots are unavailable")
    )
    return () => undefined
  }
  const subscription = liveQuery(() => readSnapshot(normalized)).subscribe({
    next(snapshot) {
      try {
        observer.onChange(
          applyAccountNetworkPreferenceRuntimeSnapshot(normalized, snapshot)
        )
      } catch (error) {
        setInboxCoordinatedPendingCheckpoint(normalized, null)
...
export async function resumeAccountNetworkPreferenceUpdate(
  ...
  try {
    ...
    // process-local pending write authorization. A previously validated
    // removal remains a safe restriction until durable state proves it stale.
    setInboxCoordinatedPendingCheckpoint(normalized, null)
    throw error
  }
}
