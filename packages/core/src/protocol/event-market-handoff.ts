...
export async function resolveEventMarketOrganizerInbox(
  ...
  const relayUrls = secureRelayUrls(declaration.relayUrls)
...
function eventMarketPrivateRelayTargetRef(relayUrl: string): string {
  const normalized = secureRelayUrls([relayUrl])[0]
  if (!normalized) {
...
function normalizedRelayTargetRefs(relayUrls: readonly string[]): string[] {
  return Array.from(
    new Set(
      secureRelayUrls(relayUrls).map((relayUrl) =>
...
async function strictInboxRelays(pubkey: string): Promise<string[]> {
  ...
  if (
    ...
    declaration.stale ||
    secureRelayUrls(declaration.relayUrls).length === 0
  ) {
    throw new Error("Private-message recipient inbox is not currently usable.")
  }
  return secureRelayUrls(declaration.relayUrls)
}
...
function pendingRelayUrls(
  ...
  return secureRelayUrls(relayUrls).filter(
...
function mergeAcknowledgedRelayRefs(input: {
  ...
  const attempted = new Set(secureRelayUrls(input.attemptedRelayUrls))
  const additions = secureRelayUrls(input.successfulRelayUrls)
...
function deliveryLegStatus(
  ...
  return acknowledgedCurrentCount === secureRelayUrls(currentRelayUrls).length
    ? "full_success"
    : "partial_success"
}
...
export async function retryEventMarketPrivateDelivery(input: {
  ...
  const recipientRelayUrls = input.recipientInboxRelays
    ? secureRelayUrls(input.recipientInboxRelays)
  ...
  if (input.record.signedSelfWrap) {
    try {
      const senderRelayUrls = input.senderInboxRelays
        ? secureRelayUrls(input.senderInboxRelays)
...
export type EventMarketHandoffAckGate =
  ...
  | {
      ...
      reason:
        ...
        | "decrypt_failure"
        | "inbox_not_declared"
        | "incomplete_revocation_coverage"
...
/** Missing revocation evidence on any failed planned relay blocks handoff. */
export function resolveEventMarketHandoffAckGate(input: {
  ...
  if (input.read.stale) return { state: "blocked", reason: "stale_read" }
  if (input.read.decryptFailureCount > 0) {
    return { state: "blocked", reason: "decrypt_failure" }
  }
  if (input.read.inbox?.declarationState !== "declared") {
    return { state: "blocked", reason: "inbox_not_declared" }
  }
  if (input.read.inbox.coverage !== "complete") {
    return {
      state: "blocked",
...
/** Issue a one-use ACK capability only from a complete revocation read. */
...
/** Derive the only ACK payload authorized by a complete current inbox read. */
