    ...
    <>
      ...
      <section aria-labelledby="privacy-6">
        ...
        <p>
          ...
          dedicated secret-keyed HMAC. The raw order UUID is not sent to
          PostHog. PostHog receives only the resulting opaque per-order UUID,
          the positive whole-satoshi estimated volume, the UTC calendar date, a
          shared static service identifier, and fields technically required to
          ingest the event. Person-profile processing is disabled. The
          requesting browser’s IP address and other request headers are not
          forwarded to PostHog.
        </p>
        <p>
          The opaque per-order UUID is used only to collapse shopper and
          merchant observations of the same logical order. It is not derived
          from a buyer, merchant, browser, device, invoice, or wallet and is not
          used to link different orders or create a person profile. The Worker
          applies aggregate and opaque-order rate limits without using them as
