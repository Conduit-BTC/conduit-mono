  ...
  return (
    <>
      ...
      <section aria-labelledby="privacy-3">
        ...
        <p>
          When the Product Apps process a sign-out or disconnect, or detect an
          account switch or loss of signer authority, they close that session’s
          authenticated relay connections and clear transient Product App
          authentication evidence. This does not necessarily delete
          account-scoped messages, orders, relay settings, wallet information,
          or other browser caches, which may reappear after reconnecting the
          same account. The Product Apps attempt to erase saved
          authentication-session and NIP-46 client-key material; if the app
          reports that cleanup failed, the user should clear site data.
        </p>
        <p>
      ...
      <section aria-labelledby="privacy-4">
        ...
        <p>
          For compatibility, the Product Apps may also retrieve legacy NIP-04
          direct messages and ask the user’s signer to decrypt them locally.
          They do not publish new legacy kind-4 direct messages. Kind-4 is not
          NIP-59 gift-wrapped: its sender public key, recipient <code>p</code>
          tag, ciphertext, and request filters remain visible to the queried
          relay, and the kind-1059 NIP-42 protected-read path described below
          does not protect these legacy reads. Decrypted legacy messages may be
          included in the same browser-local message cache and are subject to
          the same device, signer, relay, and counterparty risks described in
          this policy.
        </p>
        <p>
      ...
      <section aria-labelledby="privacy-5">
        ...
        <p>
          ...
          connection. A relay may observe the recipient public key in the gift
          wrap’s outer <code>p</code> tag, the encrypted gift wrap and its event
          size, timing, and traffic volume. The relay and its hosting or
          transport providers may observe ordinary connection metadata such as
          source IP, destination, timing, and traffic volume. A provider that
          terminates the WebSocket or encrypted transport path may also process
          relay payloads.
        </p>
        <p>
          When a signed-in user refreshes a private inbox, the Product Apps may
          send recipient-scoped requests to multiple planned relays, including
          declared private-inbox relays, locally enabled <code>IN</code> relays,
          and Conduit-configured bounded compatibility relays. Each queried
          relay can observe the request filter—including kind <code>1059</code>
          and the account public key in <code>#p</code>—even if it returns no
          event and even if NIP-42 is not used.
        </p>
        <p>
          If a relay issues a usable NIP-42 challenge, the Product Apps can ask
          the active NIP-07 or NIP-46 account to sign an ephemeral kind-22242
          authentication event before they send or retry the recipient-scoped
          request. The chosen signer receives the signing request. The relay
          receives the signed event’s account public key, timestamp, relay URL,
          connection-bound challenge, event id, and signature and can link them
          to the socket, source IP, and inbox query. Guest-order keys are not
          eligible for this authentication.
        </p>
        <p>
          NIP-42 limits access only when the relay actually challenges and
          correctly enforces an authorization policy. Client support, NIP-11
          capability metadata, a challenge, or a prior successful authentication
          response does not by itself prove that a relay prevents every
          unauthorized request. During compatibility rollout, a relay that does
          not challenge may still return encrypted gift wraps without
          authentication. NIP-42 proves control of a Nostr key to that relay; it
          does not verify real-world identity, encrypt message contents, conceal
          network metadata from the relay, or make the relay trusted. Product
          Apps continue to use NIP-44/NIP-59 encryption and distinguish
          unavailable or partial relay reads from an empty inbox.
        </p>
        <p>
          Ordinary public product, profile, listing, relay-list, storefront, and
          private-inbox declaration reads do not intentionally send an NIP-42
          account proof or prompt the signer. They are not anonymous at the
          network layer: relays and their providers still receive ordinary
          connection metadata, and each relay receives request filters that may
          contain public keys, event ids, kinds, or event coordinates.
        </p>
        <p>
      ...
      <section aria-labelledby="privacy-6">
        <h2 id="privacy-6">
          6. Product Telemetry And Aggregate Commerce Measurement
        </h2>
        <p>
          Optional browser analytics for the official Product Apps are separate
          from analytics used on the informational Conduit Website. When
          configured and enabled, this optional telemetry uses explicitly
          instrumented operational events; sanitized route classes; coarse
          status, count, amount, time, and latency buckets; pageview and
          pageleave timing; and bounded Web Vitals. Browser telemetry uses one
          shared static service identifier and temporary session and pageview
          identifiers. It is not intended to identify a person or a durable
          Product App account.
        </p>
        <p>
        ...
        <p>
          Optional browser analytics do not create or read analytics cookies and
          do not use localStorage analytics persistence, persistent account or
          visitor identifiers, person profiles, automatic interaction capture,
          heatmaps, console capture, or session replay. They remove or prohibit
        ...
        <p>
          Optional browser analytics on the official Product App hosts are
          limited to PostHog and pass through a Conduit-operated Cloudflare
          proxy. Conduit and Cloudflare necessarily process ordinary connection
        ...
        <p>
          Conduit also measures aggregate estimated commerce volume as a
          narrowly scoped first-party service metric. Because commerce is
        ...
        <p>
          ...
          three fields and immediately transforms the random order UUID with a
          dedicated secret-keyed HMAC. For daily aggregation, the HMAC input and
          domain are scoped to the order’s UTC calendar date. The raw order UUID
          exists only transiently in the request handler. It is not placed in
          durable storage or sent to PostHog.
        </p>
        <p>
          Until daily aggregation is activated for a future UTC cutover, the
          bounded prior path sends PostHog an opaque per-order UUID, the
          estimated amount, the UTC order date, and a shared static service
          identifier. It does not send the raw order UUID or an actor
          identifier. One observation is not sent through both the prior and
          daily paths.
        </p>
        <p>
          After activation, the Worker uses one daily aggregate store for each
          UTC order date on or after the cutover. It keeps only the
          secret-derived opaque per-order fingerprint needed to collapse shopper
          and merchant observations of the same logical order, together with the
          day’s aggregate estimated amount and delivery state. It does not keep
          a per-order amount. The fingerprint is not derived from a buyer,
          merchant, browser, device, invoice, or wallet, is scoped to one order
          date, and is not used to link different orders or create a person
          profile. The Worker also applies aggregate and opaque-order rate
          limits without using them as durable user identifiers.
        </p>
        <p>
          PostHog receives immutable aggregate revision snapshots for the UTC
          order date. Each snapshot contains an opaque event UUID, the positive
          whole-satoshi daily estimated total, the UTC calendar date, a shared
          static service identifier, and fields technically required to ingest
          the event. It does not receive an order UUID, opaque per-order
          fingerprint, or per-order amount. The first snapshot is scheduled 12
          hours after the UTC order date closes, and later reconciliation is
          batched into fixed 12-hour windows rather than forwarded one
          observation at a time. If a delivery fails or its result is unclear,
          the Worker retries the same frozen snapshot with the same UUID;
          estimates accepted meanwhile wait for the next fixed window, which
          uses a different UUID. Multiple snapshots may remain for one UTC date.
          Reporting selects the greatest total for each UTC order date and must
          not sum those snapshots. Person-profile processing is disabled. The
          requesting browser’s IP address and other request headers are not
          forwarded to PostHog.
        </p>
        <p>
          Duplicate observations of one order are designed to collapse before
          daily aggregation. If shopper and merchant observations disagree on
          the amount, the first accepted positive estimate is retained. A later
          accepted order for the same UTC date produces a later aggregate
          snapshot with the updated daily total; an earlier revision may remain
          in PostHog and is excluded by selecting the greatest total for that
          date. For up to 30 days after daily aggregation is activated,
          observations for earlier order dates may continue through the prior
          path, under which PostHog receives an opaque per-order UUID, estimated
          amount, UTC date, and the same static service identity.
        </p>
        <p>
          Conduit uses these events only to calculate aggregate estimated
          Conduit commerce gross merchandise volume. A shopper report is an
          estimate signal, not settlement proof. Merchant and shopper reports
          may be mistaken, incomplete, repeated, or manipulated. The aggregate
          can therefore overcount or undercount actual paid commerce. This
      ...
      <section aria-labelledby="privacy-8">
        ...
        <ul>
          ...
          <li>
            calculate aggregate estimated Conduit commerce gross merchandise
            volume;
          </li>
      ...
      <section aria-labelledby="privacy-10">
        ...
        <p>
          When daily aggregation applies, active opaque per-order fingerprints
          used for aggregate commerce deduplication are retained through 30 days
          after the UTC order date and are then deleted from active storage even
          if a final best-effort aggregate snapshot cannot be delivered.
          Cloudflare’s SQLite recovery systems may keep restorable copies within
          a separate provider-managed point-in-time recovery window, currently
          up to 30 days. PostHog’s aggregate event retention is described in
          Section 6.
        </p>
      </section>
      ...
      <section aria-labelledby="privacy-13">
        ...
        <p>
          The Global Privacy Control (“GPC”) is treated as a request to opt out
          of sale or sharing of personal information where applicable. Conduit
          does not sell personal information or share it for cross-context
          behavioral advertising. The Product Apps also use GPC to suppress
          optional browser analytics. GPC does not suppress the first-party
          aggregate commerce measurement described in Section 6 because that
          measurement is not used to sell or share personal information, serve
          behavioral advertising, create a person profile, or identify a buyer
          or merchant.
        </p>
      </section>
    ...
    <>
      ...
      <section aria-labelledby="terms-11">
        ...
        <p>
          ...
          messages locally in the browser or through the user’s signer. For
          compatibility, they may also retrieve legacy NIP-04 direct messages
          and ask the signer to decrypt them locally, but they do not publish
          new legacy kind-4 messages. The legacy event’s sender public key,
          recipient <code>p</code> tag, ciphertext, and query filters remain
          relay-visible and are not protected by the kind-1059 NIP-42 read path.
          Message content is available to the sender and recipient and may also
          be processed by their browsers, signers, wallets, counterparties, or
          chosen services.
        </p>
        <p>
          Encryption does not eliminate routing and network metadata. Relays may
          observe outer recipient tags, encrypted event size, timing, traffic
          volume, selected relays, and request information. Relays and their
          hosting or transport providers may observe connection behavior and IP
          addresses. When a signed-in user reads a protected private inbox,
          every queried relay can observe the recipient-scoped filter. A relay
          that issues a usable challenge may also receive an NIP-42
          authentication event from the user’s active NIP-07 or NIP-46 account
          before the Product App sends or retries the request. Ordinary public
          reads do not request that account proof, but they are not anonymous at
          the network layer. NIP-42 client support does not guarantee that every
          relay challenges, authorizes, or enforces recipient-only access, and a
          non-challenging relay may return encrypted gift wraps without
          authentication. Messages may be delayed, rejected, duplicated,
          retained, deleted, or never retrieved. Conduit does not guarantee
          confidentiality against a compromised device, signer, key,
          counterparty, browser, relay, or implementation.
        </p>
      </section>
