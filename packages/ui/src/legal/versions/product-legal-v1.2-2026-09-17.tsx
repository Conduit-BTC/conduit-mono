  ...
  return (
    <>
      ...
      <section aria-labelledby="privacy-6">
        ...
        <p>
          After the receipt-authority boundary verifies a completed public Zap
          Out, it may also emit a settled-volume event directly to PostHog. That
          event contains the exact positive whole-satoshi invoice amount, a
          timestamp rounded to the UTC calendar day, a shared static service
          identifier, and a secret-key-derived opaque identifier used only to
          suppress duplicate reporting of the same verified receipt. It does not
          include buyer or merchant identifiers, public keys, product or order
          coordinates, comments, invoices or payment requests, payment hashes or
          preimages, receipt or event identifiers, relay or wallet details,
          connection secrets, fees, or the requesting browser's IP address.
          PostHog person-profile processing is disabled for the event.
        </p>
        <p>
          Conduit uses this event only to calculate aggregate settled volume for
          verified public Zap Outs. Reporting is best effort and may undercount.
          It does not determine payment state or prove sales, merchant
          acceptance, fulfillment, refunds, or final merchant revenue. Because
          the amount is exact and public zap receipts are public Nostr events,
          an unusually distinctive amount and day may be correlatable with
      ...
      <section aria-labelledby="privacy-7">
        ...
        <p>
          When Conduit Shop asks the same boundary to verify a completed public
          Zap Out, the endpoint receives the public zap receipt and its embedded
          zap request, including the public provider and recipient identifiers
          and the BOLT11 invoice required to verify receipt authority and the
          exact paid amount. The endpoint uses that public protocol material for
          verification and duplicate suppression. It does not forward the
          receipt, request, invoice, or public identifiers to PostHog.
        </p>
      </section>
