...
Direct activation briefly replaces the icon with the compact label System, Day, or Night inside the same 44px button. The selected icon returns after about 400ms: feedback starts fading out at 320ms with an 80ms transition. Repeated clicks update the label and restart the timing without moving focus or adding a popup. Accessible names and polite announcements retain the full preference names. Automatic system and cross-tab updates do not trigger feedback; reduced-motion users receive the same confirmation without animation.
