...
For `organizer_handoff`, after the merchant attests that a non-zero order is
paid, or immediately for a zero-cost order, the merchant may publish a separate
minimal NIP-17/NIP-59 private rumor to the organizer. The rumor type is
...
organizer-handoff graph. Catalog acceptance alone, a merchant request alone, or
pickup authorship alone is insufficient.
...
A positively observed ready receipt may be shown under degraded inbox coverage,
but the irreversible `handed_out` action requires a complete bounded read of
the planned receipt/revocation sources. Otherwise a valid revocation could be
hidden on the unavailable source. The UI exposes that uncertainty and retries;
it never turns incomplete negative evidence into permission to hand out.
...
- Non-zero orders cannot emit a ready receipt before merchant-confirmed payment;
  zero-cost orders can be attested ready without a payment event.
- Ready receipts carry only the strict graph, quantities, an empty reserved
  options field, and the opaque claim field. Privacy tests reject buyer
...
- A ready receipt observed with incomplete revocation coverage remains visible
  but cannot be acknowledged until the bounded read is complete.
- A valid revocation removes readiness; a revoked receipt cannot be handed out;
  a valid organizer acknowledgement cannot grant any merchant-only lifecycle
  authority.
