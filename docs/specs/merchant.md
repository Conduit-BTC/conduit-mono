...
const templates = [
  {
    ...
    text: "Thank you for your order! I'll send an invoice shortly.",
  },
  { label: "Shipped", text: "Your order has shipped! Tracking: {tracking}" },
  {
    label: "Delay Notice",
    text: "Sorry for the delay. Expected ship date: {date}",
  },
]
```
...
async function loadProfileWithRelayDiscovery(pubkey: string) {
  ...
  const relays = relayList?.length > 0 ? relayList : PUBLIC_FALLBACK_RELAYS
