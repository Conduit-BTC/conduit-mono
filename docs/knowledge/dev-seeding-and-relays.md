...
```bash
CHECK_RELAY_URLS=ws://127.0.0.1:7777 bun run check:products
```
...
- `SEED_RELAY_URLS` (optional): comma-separated relay URLs
  - If omitted, falls back to `VITE_DEFAULT_RELAY_URL`, else the local relay at `ws://127.0.0.1:7777`.
...
```bash
SEED_NSEC=... \
SEED_RELAY_URLS=wss://relay.plebeian.market,wss://relay.ditto.pub \
...
SEED_NSEC=... SEED_RELAY_URLS=wss://relay.plebeian.market,wss://relay.ditto.pub bun run seed:products

# 2) set Cloudflare Pages env var for the project:
# VITE_DEFAULT_RELAYS=wss://relay.plebeian.market,wss://relay.ditto.pub
```
