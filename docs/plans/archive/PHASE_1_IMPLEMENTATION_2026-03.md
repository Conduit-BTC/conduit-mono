# Archived Implementation Guide

> Archived historical Phase 1 planning record. Do not use this document for current execution, route maps, env values, relay URLs, payment phase labels, or design tokens.
...
- [x] Color palette from `/conduit-market-client/src/styles/`
- [x] Component patterns from `/conduit-market-client/src/components/`
- [x] Typography/spacing from Figma
...
- [x] All event kind definitions with tag schemas
- [x] NIP-17 wrapping flow (seal + gift wrap)
- [x] Order event types (order, payment_request, status_update, shipping_update, receipt)
- [x] Validation with `@conduit/core` Zod schemas (product/order/etc)
- [x] Payment flow (NWC invoice generation, zap receipts)
...
- [x] Color palette (dark theme, purple/violet primary, lime secondary)
- [x] Typography scale (ABC Whyte Inktrap for headings, Poppins for body)
- [x] Spacing and layout grid
- [x] Component designs (cards, buttons, forms, nav)
- [x] Screen layouts for each route
...
- [x] Confirm NDK version compatibility
- [x] List other reusable packages from legacy
...
export const config = {
  ...
  defaultRelays: (
    import.meta.env.VITE_DEFAULT_RELAYS ?? "wss://relay.damus.io,wss://nos.lol"
  ).split(","),
  lightningNetwork: (import.meta.env.VITE_LIGHTNING_NETWORK ??
    "mock") as LightningNetwork,
...
- [x] Verify preview deploys work from GitHub Actions (test with first PR)
...
Prepare the Conduit codebase for public open-source contribution without forcing proprietary products into the same repository.

- [ ] Keep the public Conduit repo canonical for Market, Merchant, `@conduit/core`, and `@conduit/ui`
- [ ] Move Store Builder into private `conduit-services`
- [ ] Move internal-only planning docs out of tracked public paths
- [ ] Keep public specs/architecture/contributor docs in the public repo
- [ ] Add public trust surfaces (`About`, repo links, contributor visibility, admin/operator pubkeys)
...
Historical reference now lives at `docs/knowledge/future/store-builder.md`.
...
Historical reference now lives in `docs/knowledge/future/monetization.md` and `docs/knowledge/future/billing.md`.
...
- `stores` - Store Builder billing (linked from the future Store Builder note)
...
Historical billing reference now lives at `docs/knowledge/future/billing.md`.
...
- [x] Market deployed to shop.conduit.market
- [x] Portal deployed to sell.conduit.market
...
- [x] Work through the MVP hi-fi worklist in `docs/plans/archive/HI_FI_CHECKLIST.md`
- [x] Extract tokens (color/typography/spacing) from Figma into `packages/ui`
- [x] Align shared shells, auth flows, and core components with Figma
- [x] Bring MVP Market flows in line with Figma (see `docs/plans/archive/HI_FI_CHECKLIST.md`)
- [x] Bring MVP Merchant flows in line with Figma (see `docs/plans/archive/HI_FI_CHECKLIST.md`)
...
- [x] Merchant orders/products visual polish
- [x] Triage post-MVP hi-fi backlog after MVP using `docs/plans/PHASE_2_IMPLEMENTATION.md`
