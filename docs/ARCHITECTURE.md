...
localStorage is used for small local preferences and cart state. Sensitive
order/message/payment contents should stay in the encrypted message flow or
local IndexedDB records designed for that purpose. A legacy single-wallet NWC
record may be read only for transactional migration into `wallets` and
`walletCredentials`; new wallet credentials must not be written to
localStorage.
