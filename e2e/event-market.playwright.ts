...
test("verified event catalog enables a completed merchant while another exact product read is held @market", async ({
  page,
}) => {
  test.setTimeout(120_000)
  const relay = createRelayHarness()
...
test("organizer offer off publishes an empty catalog and permits booth handoff @market @merchant", async ({
  ...
  await expect(eventsHeading).toHaveCSS("visibility", "hidden")
  ...
  await expect(
    printPreview.locator('[data-event-sign-page-count="1"]')
  ).toBeVisible()
  await printPreview.getByRole("button", { name: "Close" }).click()
