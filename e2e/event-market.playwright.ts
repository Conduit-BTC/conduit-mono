...
const BUYER_SECRET = new Uint8Array(32).fill(53)
...
const MERCHANT_PRODUCT_D_TAG = "synthetic-merchant-handoff-product"
const MERCHANT_PRODUCT_COORDINATE = `30402:${MERCHANT_PUBKEY}:${MERCHANT_PRODUCT_D_TAG}`
const MERCHANT_PRODUCT_TITLE = "Synthetic merchant booth mug"
const MERCHANT_PICKUP_D_TAG = "synthetic-merchant-booth"
const MERCHANT_PICKUP_COORDINATE = `30406:${MERCHANT_PUBKEY}:${MERCHANT_PICKUP_D_TAG}`
const FIXTURE_RELAY = "wss://event-market-browser.example"
...
async function installSyntheticEnvironment(
  ...
  await page.route("https://event-market-browser.example/**", (route) =>
    route.fulfill({
      status: 200,
...
async function publishOrganizerMarket(
  ...
  await expect(
    page.getByRole("heading", { name: "Event markets", exact: true })
  ).toBeVisible()
  ...
  await editor
    ...
    .fill("https://example.com/synthetic-event-market.svg")
  await editor
  ...
  const organizerOffer = editor.getByRole("checkbox", {
    name: "Organizer can hand out merchant products",
  })
  await expect(organizerOffer).not.toBeChecked()
  if (options.organizerHandoffEnabled) {
    await organizerOffer.check()
    await editor.getByLabel("Pickup title").fill("Synthetic main entrance")
    await editor
      .getByLabel("Public handoff location")
      .fill("Synthetic Fixture Hall entrance")
    await editor.getByLabel("Country code").fill("US")
    await editor.getByLabel("Pickup price").fill("0")
    await editor.getByLabel("Currency", { exact: true }).fill("SAT")
  } else {
    await expect(editor.getByLabel("Pickup title")).toHaveCount(0)
  }
...
function createMerchantPickupEvent(createdAt: number): SignedEvent {
  return signEvent(MERCHANT_SECRET, {
    kind: 30406,
    created_at: createdAt,
    content: "Synthetic merchant booth pickup.",
    tags: [
      ["d", MERCHANT_PICKUP_D_TAG],
      ["title", "Synthetic merchant booth"],
      ["price", "0", "SAT"],
      ["country", "US"],
      ["service", "pickup"],
      ["location", "Synthetic Fixture Hall, Booth 12"],
    ],
  })
}
...
function createMerchantProductEvent(input: {
  ...
  return signEvent(MERCHANT_SECRET, {
    ...
    tags: [
      ...
      ["stock", "3"],
      ["image", "https://example.com/synthetic-pickup-product.svg"],
...
test("organizer offer off publishes an empty catalog and permits booth handoff", async ({
  ...
  await expect(
    page.getByText(
      "Organizer handoff is not offered. Accepted merchants may provide their own signed booth pickup.",
      { exact: true }
    )
  ).toBeVisible()
  await expect(
  ...
  const merchantPickup = createMerchantPickupEvent(
    market.initialCollection.created_at + 1
  )
  const merchantProduct = createMerchantProductEvent({
    dTag: MERCHANT_PRODUCT_D_TAG,
    title: MERCHANT_PRODUCT_TITLE,
  ...
  expect(eventCoordinate(merchantProduct)).toBe(MERCHANT_PRODUCT_COORDINATE)
...
test("organizer handoff completes a private order receipt and exact ACK flow", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText("Minimal organizer pickup receipt delivered", {
      exact: true,
    })
  ).toBeVisible({ timeout: 30_000 })
  await expect
