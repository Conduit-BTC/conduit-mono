...
describe("commerce gateway", () => {
  ...
  it("uses an explicit conversation signer when the global NDK signer is absent", async () => {
    ...
    const result = await getBuyerConversationList({
      ...
      limit: 50,
      signer: guestSigner,
      expectedOrderId: "guest-order-1",
      expectedCounterpartyPubkey: "merchant",
    })
  ...
  it("rejects foreign orders and senders during scoped guest recovery", async () => {
    const guestSigner = { id: "scoped-guest-signer" } as never
    const wrappedEvents = [
      {
        id: "wrap-scoped-valid",
        kind: EVENT_KINDS.GIFT_WRAP,
        tags: [["p", "scoped-guest"]],
      },
      {
        id: "wrap-scoped-foreign-order",
        kind: EVENT_KINDS.GIFT_WRAP,
        tags: [["p", "scoped-guest"]],
      },
      {
        id: "wrap-scoped-foreign-sender",
        kind: EVENT_KINDS.GIFT_WRAP,
        tags: [["p", "scoped-guest"]],
      },
    ]
    const rumor = (id: string, sender: string, orderId: string) => ({
      id,
      kind: EVENT_KINDS.ORDER,
      pubkey: sender,
      created_at: 101,
    ...
    __setCommerceTestOverrides({
      ...
      fetchEventsFanout: async (filter) =>
        filter.kinds?.includes(EVENT_KINDS.GIFT_WRAP)
          ? (wrappedEvents as never)
          : [],
      giftUnwrap: async (event) => {
        if (event.id === "wrap-scoped-valid") {
          return rumor("valid-rumor", "merchant", "scoped-order") as never
        }
        if (event.id === "wrap-scoped-foreign-order") {
          return rumor(
            "foreign-order-rumor",
            "merchant",
            "other-order"
    ...
    const result = await getBuyerConversationList({
      principalPubkey: "scoped-guest",
      signer: guestSigner,
      expectedOrderId: "scoped-order",
      expectedCounterpartyPubkey: "merchant",
    })

    expect(result.data).toHaveLength(1)
    expect(result.data[0]?.orderId).toBe("scoped-order")
    expect(result.data[0]?.merchantPubkey).toBe("merchant")
