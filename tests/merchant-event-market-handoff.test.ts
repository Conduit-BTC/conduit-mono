...
class MemoryStorage implements Storage {
  ...
  private nextSetItemHook?: (key: string, value: string) => void
  private nextSetItemFailure?: (key: string, value: string) => boolean
  get length() {
  ...
  setItem(key: string, value: string) {
    if (this.nextSetItemFailure?.(key, value)) {
      this.nextSetItemFailure = undefined
      throw new Error("Injected storage interruption")
    }
    this.values.set(key, value)
    this.writes.push(value)
    const hook = this.nextSetItemHook
    this.nextSetItemHook = undefined
    hook?.(key, value)
  }
  onNextSetItem(hook: (key: string, value: string) => void) {
    this.nextSetItemHook = hook
  }
  failNextSetItemWhere(predicate: (key: string, value: string) => boolean) {
    this.nextSetItemFailure = predicate
  }
  serialized() {
...
describe("merchant organizer handoff workflow", () => {
  ...
  it("preserves a found exact ACK while reporting degraded discovery", () => {
    const readyReceiptId = "6".repeat(64)
    const expectedGraph = {
      claimRef: "8".repeat(64),
      merchantPubkey: MERCHANT,
    ...
    for (const result of [
      ...
      expect(result.exactAck).toBe(exact)
      expect(result.conflicting).toBe(false)
    }
  })

  it("requires explicit merchant confirmation before creating a ready authorization", async () => {
        ...
        transport: {
          ...
          publishFn: (async (_event, options) => {
            ...
            return plannerResult({
              ...
              successful: options.exclusiveRelayUrls ?? [],
              failed: [],
            })
          }) as never,
        },
      })
    ).rejects.toThrow("confirm payment")
  ...
  it("releases completed ready receipts without filling delivery or marker storage", () => {
    const storage = new MemoryStorage()
    const template = readyDelivery()

    for (let index = 0; index < 100; index += 1) {
      const completed = deliveryWithIdentity(template, index)
      rememberEventMarketHandoffDelivery(MERCHANT, completed, storage)
      expect(
        releaseCompletedEventMarketHandoffReceipt(
          MERCHANT,
          completed.record.readyReceiptId,
          completed.record.orderCorrelationRef!,
          storage
        )
      ).toBe(true)
    }
    expect(storage.length).toBeLessThanOrEqual(2)

    const next = deliveryWithIdentity(template, 100)
    expect(() =>
      rememberEventMarketHandoffDelivery(MERCHANT, next, storage)
    ).not.toThrow()
    const retained = loadEventMarketHandoffDeliveries(MERCHANT, storage)
    expect(retained).toHaveLength(1)
    expect(retained[0]?.record.readyReceiptId).toBe(next.record.readyReceiptId)
  }, 10_000)

  it("preserves retryable ready receipts and revocation rows on completion cleanup", () => {
    const storage = new MemoryStorage()
    const template = readyDelivery()
    const completed = deliveryWithIdentity(template, 1)
    const retryable = deliveryWithIdentity(
      template,
      2,
      {
        status: "partial_success",
    ...
    const revocation = revocationDelivery(deliveryWithIdentity(template, 3))
    seedDeliveries(storage, [completed, retryable, revocation])

    expect(
      releaseCompletedEventMarketHandoffReceipt(
        MERCHANT,
        retryable.record.readyReceiptId,
        retryable.record.orderCorrelationRef!,
        storage
      )
    ).toBe(false)
    expect(
    ...
    expect(
      retained.some(
        (delivery) =>
          ...
          delivery.record.readyReceiptId === retryable.record.readyReceiptId
      )
    ).toBe(true)
    expect(
      retained.some(
        (delivery) =>
          ...
          delivery.record.readyReceiptId === revocation.record.readyReceiptId
      )
    ).toBe(true)
  })

  it("does not clobber a concurrent revocation upsert when releasing a completed receipt", () => {
    const storage = new MemoryStorage()
    const completed = deliveryWithIdentity(readyDelivery(), 1)
    const concurrentRevocation = revocationDelivery(
      deliveryWithIdentity(readyDelivery(), 2)
    )
    seedDeliveries(storage, [completed])
    storage.onNextSetItem(() => {
      rememberEventMarketHandoffDelivery(
        MERCHANT,
    ...
    const retained = loadEventMarketHandoffDeliveries(MERCHANT, storage)
    expect(retained).toHaveLength(1)
    expect(retained[0]?.record.messageType).toBe(
      "organizer_fulfillment_revocation"
    )
    expect(retained[0]?.record.readyReceiptId).toBe(
      concurrentRevocation.record.readyReceiptId
    )

    const laterReady = deliveryWithIdentity(readyDelivery(), 3)
    rememberEventMarketHandoffDelivery(MERCHANT, laterReady, storage)
    expect(storage.length).toBe(1)
    expect(loadEventMarketHandoffDeliveries(MERCHANT, storage)).toHaveLength(2)
  })

  it("collects an interrupted pending completion marker on the next upsert", () => {
    const storage = new MemoryStorage()
    const completed = deliveryWithIdentity(readyDelivery(), 1)
    seedDeliveries(storage, [completed])
    storage.failNextSetItemWhere((_key, value) => value === "complete")
    ...
    const laterReady = deliveryWithIdentity(readyDelivery(), 2)
    rememberEventMarketHandoffDelivery(MERCHANT, laterReady, storage)
    expect(storage.length).toBe(1)
    const retained = loadEventMarketHandoffDeliveries(MERCHANT, storage)
    expect(retained).toHaveLength(1)
    expect(retained[0]?.record.readyReceiptId).toBe(
      laterReady.record.readyReceiptId
    )
  })
  ...
  it("allows revocation without completeness proof but never over a known acknowledgement", async () => {
    const storage = new MemoryStorage()
    const ready = readyDelivery()
    rememberEventMarketHandoffDelivery(MERCHANT, ready, storage)
    rememberEventMarketHandoffDelivery(
    ...
    await expect(
      revokeOrganizerReadyReceipt({
        ...
        orderId: order().id,
        signer: {} as never,
        matchingAckReceiptIds: new Set([ready.record.readyReceiptId]),
        storage,
      })
