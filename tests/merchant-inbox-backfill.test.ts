...
describe("merchant priority inbox backfill", () => {
  ...
  it("treats a planned skipped relay as partial and rescans when it returns", async () => {
    ...
    expect(seenUntil).toEqual([undefined, undefined, 601])
  ...
  it("runs a queued deep sync after an ordinary shallow read failure", async () => {
    const firstPage = recentPage(1_000)
    const boundary = firstPage[firstPage.length - 1]!
    const olderOrderWrap = wrapEvent("recovered-older-order", 600)
    let fetchCalls = 0
    let releaseFirstPage: (() => void) | undefined
    let markFirstPageStarted: (() => void) | undefined
    const firstPageStarted = new Promise<void>((resolve) => {
    ...
    __setCommerceTestOverrides({
      fetchEventsFanoutWithDiagnostics: async (filter, options) => {
        ...
        if (fetchCalls === 1) {
          markFirstPageStarted?.()
          await firstPageGate
          throw new Error("temporary shallow read failure")
        }
        return filter.until === undefined
          ? successfulPage(firstPage, options?.relayUrls ?? [])
          : successfulPage([boundary, olderOrderWrap], options?.relayUrls ?? [])
      ...
      giftUnwrap: async (event) =>
        (event.id === olderOrderWrap.id
          ? orderRumor("order-after-shallow-failure", 600)
          : ignoredRumor(event.id)) as never,
    })
    ...
    const [shallowResult, firstDeep, secondDeep] = await Promise.allSettled([
      shallow,
      deepA,
      deepB,
    ])

    expect(shallowResult.status).toBe("rejected")
    expect(firstDeep.status).toBe("fulfilled")
    expect(secondDeep.status).toBe("fulfilled")
    expect(fetchCalls).toBe(3)
    if (firstDeep.status === "fulfilled") {
      expect(firstDeep.value.data[0]?.orderId).toBe(
        "order-after-shallow-failure"
      )
    }
    if (secondDeep.status === "fulfilled") {
      expect(secondDeep.value.meta.inbox?.historyCoverage).toBe(
