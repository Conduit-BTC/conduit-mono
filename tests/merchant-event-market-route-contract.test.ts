...
describe("merchant organizer event market route", () => {
  ...
  it("makes event authoring requirements and modal state explicit", async () => {
    ...
    expect(editor).toContain("https://nostrmedia.com/")
    expect(editor).toContain("sign in with your Nostr extension")
    expect(editor).toContain("upload a public")
    expect(editor).toContain("Copy its direct HTTPS image URL")
