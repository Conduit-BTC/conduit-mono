...
const INBOX_C = "wss://inbox-c.example"
...
describe("coordinated account Network updates", () => {
  ...
  it("signs a reviewed empty inbox replacement for a declared frontier", async () => {
    ...
    const result = await publishAccountNetworkPreferenceUpdate({
      pubkey: OWNER,
      action: {
        ...setRolesAction({ inboxUrls: [] }),
        allowSignedEmptyInbox: true,
      },
    ...
    expect(signedKinds).toEqual([10050])
    expect(result.update?.checkpoints[0]?.signedEvent.tags).toEqual([])
  })
  ...
  it("puts signed NIP-65 write relays first in the default bounded plan", async () => {
    const fixture = baseFixture()
    const updateDependencies = dependencies(fixture.repository, fixture.current)
    delete updateDependencies.resolveRelayPlan

    const result = await publishAccountNetworkPreferenceUpdate({
      pubkey: OWNER,
      action: setRolesAction({ nip65Urls: [PUBLIC_B, PUBLIC_C] }),
      signer: signer(),
      reviewed: reviewAccountNetworkPreferences(fixture.current),
    ...
    expect(relayList.relayPlan.slice(0, 2)).toEqual([PUBLIC_B, PUBLIC_C])
  })
  ...
  it("keeps stronger relay evidence when a weaker concurrent result arrives late", async () => {
    const fixture = baseFixture()
    ...
    const checkpoint = first.update!.checkpoints[0]!
    const initial = checkpoint.relayOutcomes[0]!
    const initialCompleteObservedAt =
      fixture.repository.owner!.current!.completeObservedAt
    const initialLookupObservedAt =
      fixture.repository.owner!.latestLookup!.observedAt

    const late = await fixture.repository.recordOutcomes({
      pubkey: OWNER,
      updateId: first.update!.updateId,
      publish: [
        {
          kind: checkpoint.kind,
          relayUrl: initial.relayUrl,
          status: "timed_out",
        },
      ],
      readback: [
    ...
    const retained = late.record.checkpoints[0]!.relayOutcomes[0]!

    expect(retained).toMatchObject({
      publishStatus: "acked",
      publishAttemptCount: 2,
      publishAttemptedAt: initial.publishAttemptedAt,
      readbackStatus: "observed",
      readbackAttemptCount: 2,
      readbackAttemptedAt: initial.readbackAttemptedAt,
      observedAt: initial.observedAt,
    })
    expect(fixture.repository.owner!.current!.completeObservedAt).toBe(
      initialCompleteObservedAt
    )
    expect(fixture.repository.owner!.latestLookup!.observedAt).toBe(
      initialLookupObservedAt
    )
    expect(late.record.updatedAt).toBe(NOW + 1_000)
  })
  ...
  it("carries an observed unchanged checkpoint and its pending inbox authority into a one-kind successor", async () => {
    ...
    const retainedCurrent = (await readRetainedInboxDeclaration(OWNER, {
      evidenceRepository: inboxRepository(fixture.repository),
      now: () => clock,
    }))!
    current.inboxDeclaration = {
      ...retainedCurrent,
      observation: current.inboxDeclaration.observation,
    }
    expect(current.inboxDeclaration).toMatchObject({
  ...
  it("carries every active recovery inbox into a pending B-to-C successor", async () => {
    ...
    const firstInbox = first.update!.checkpoints.find(
      (checkpoint) => checkpoint.kind === 10050
    )!
    const current = reconciliation({
      relayList: fixture.relayList,
      inbox: firstInbox.signedEvent,
    })
    const retainedCurrent = (await readRetainedInboxDeclaration(OWNER, {
      evidenceRepository: inboxRepository(fixture.repository),
    ...
    const successor = await publishAccountNetworkPreferenceUpdate({
      pubkey: OWNER,
      action: setRolesAction({ inboxUrls: [INBOX_C] }),
      signer: signer(),
      reviewed: reviewAccountNetworkPreferences(current),
      dependencies: dependencies(fixture.repository, current, {
        now: () => NOW + 1_000,
    ...
    expect(successor.update?.previousInboxRelayUrls).toEqual([INBOX_B, INBOX_A])
  ...
  it("hydrates the pending runtime overlay before fresh relay reconciliation", async () => {
    const fixture = baseFixture()
    await publishAccountNetworkPreferenceUpdate({
      pubkey: OWNER,
      action: setRolesAction({ nip65Urls: [PUBLIC_A, PUBLIC_B, PUBLIC_C] }),
    ...
    expect(hydrated.pendingUpdate?.updateId).toBe(restarted.update?.updateId)
    expect(hydrated.pendingUpdateStatus).toBe("ready")
    expect(
      getAccountRelaySettingsProjection(
        getAccountRelayScope(OWNER)
      )?.entries.map((entry) => entry.url)
  ...
  it("whole removal cuts off only the named URL immediately and after restart", async () => {
    ...
    let readPlan = planInboxReadRelays({
      declaration: retained!,
      authenticatedPubkey: OWNER,
      // Simulate a caller that captured its durable snapshot before another
      // tab staged the removal. The newer process projection must still win.
  ...
  it("keeps a validated removal cutoff when a later resume read fails", async () => {
    const fixture = baseFixture({
      nip65Urls: [REMOVED, PUBLIC_A, PUBLIC_B],
      inboxUrls: [REMOVED, INBOX_B],
    })
    await publishAccountNetworkPreferenceUpdate({
      pubkey: OWNER,
      action: { type: "remove_relay", relayUrl: REMOVED },
    ...
    const unavailable = fixture.repository.snapshot()
    unavailable.get = async () => {
      throw new Error("update store unavailable")
    }
    await expect(
      resumeAccountNetworkPreferenceUpdate(OWNER, unavailable, () => NOW + 1)
    ).rejects.toThrow("update store unavailable")

    setInboxMigrationRecoveryRelayUrls(OWNER, [REMOVED, INBOX_A])
    const retained = await readRetainedInboxDeclaration(OWNER, {
      evidenceRepository: inboxRepository(fixture.repository),
      now: () => NOW + 1,
    })
  ...
  it("reloads an account-scoped removal cutoff without trusting another tab's process state", async () => {
    const fixture = baseFixture({
      nip65Urls: [REMOVED, PUBLIC_A, PUBLIC_B],
      inboxUrls: [REMOVED, INBOX_B],
    })
    ...
    await expect(
      loadAccountPrivateMessageRelayCutoff(OWNER, restarted)
    ).resolves.toEqual({ pubkey: OWNER, excludedRelayUrls: [REMOVED] })
    await expect(
      loadAccountPrivateMessageRelayCutoff(
        OTHER,
        new MemoryNetworkUpdateRepository({})
      )
    ).resolves.toEqual({ pubkey: OTHER, excludedRelayUrls: [] })
    expect(getInboxExplicitRemovalRelayUrls(OWNER)).toEqual([REMOVED])

    restarted.inbox = inboxEvidence(
      signedEvent(
        10050,
        [
          ["relay", REMOVED],
        ...
        Math.floor(NOW / 1_000) + 100
      )
    )
    await expect(
      loadAccountPrivateMessageRelayCutoff(OWNER, restarted)
    ).resolves.toEqual({ pubkey: OWNER, excludedRelayUrls: [] })
  })
