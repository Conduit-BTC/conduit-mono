...
import NDK, { NDKEvent, NDKPrivateKeySigner, nip19, type NDKFilter } from "@nostr-dev-kit/ndk"
import { EVENT_KINDS, parseProductEvent, type ProductSchema } from "@conduit/core"
...
async function pickFirstReachableRelayUrl(urls: string[]): Promise<string | null> {
...
describeIfRelay("merchant products CRUD (relay smoke)", () => {
  it(
    "publishes, updates (same d tag), and deletes a kind 30402 listing",
    async () => {
