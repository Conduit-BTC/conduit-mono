...
describeIfRelay("merchant products CRUD (relay smoke)", () => {
  it("publishes, updates (same d tag), and deletes a kind 30402 listing", async () => {
    ...
    const draftV1 = buildProductListingEventDraft({
      product: productV1,
      dTag,
    })
    ev1.kind = draftV1.kind
    ev1.created_at = Math.floor(createdAt / 1000)
    ev1.content = draftV1.content
    ev1.tags = draftV1.tags
    ...
    const draftV2 = buildProductListingEventDraft({
      product: productV2,
      dTag,
    })
    ev2.kind = draftV2.kind
    ev2.created_at = Math.floor(updatedAt / 1000)
    ev2.content = draftV2.content
    ev2.tags = draftV2.tags
