...
describe("reviewed merchant payment address recovery", () => {
  for (const scenario of [
    ...
    it(`keeps the recovery boundary safe: ${scenario}`, async () => {
      const orderId = `updated-address-${scenario}`
      ...
      const dependencies = paymentDependencies({
        ...
        claimOrderLifecycleUpdatedAddressPayment: async (
          ...
          claims += 1
          expect(input.merchantLightningAddress).toBe(reviewed.previousAddress)
      ...
      try {
        if (scenario === "success" || scenario === "session_during_invoice") {
          ...
          expect(stored.merchantLightningAddress).toBe(reviewed.previousAddress)
