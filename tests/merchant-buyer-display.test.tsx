...
describe("merchant buyer display", () => {
  ...
  it("shares the guest-aware order row between Merchant Home and Orders", async () => {
    ...
    expect(rowSource).toContain("<Avatar")
    expect(rowSource).toContain("<AvatarImage")
    expect(rowSource).toContain("<AvatarFallback")
    expect(rowSource).toContain('alt={decorative ? "" : name}')
    expect(rowSource).not.toContain("normalizePublicMediaUrl")
