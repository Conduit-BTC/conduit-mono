...
describe("account Network settings controller contract", () => {
  ...
  it("prepares one frozen authoritative review and exposes one execute doorway", () => {
    ...
    expect(execution).not.toContain("revisionRef.current !== preparedRevision")
    expect(preparation).not.toContain("evidenceReady")
    expect(controllerSource).toContain("!accountPreferences.localReady")
